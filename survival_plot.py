...
def plot_grid_survival(fuzzers_data, max_time, output_dir, binary):
    fuzzers_data = sorted_fuzzers_data = sorted(fuzzers_data, key=lambda fuzzer: list(fuzzer.keys())[0])
    MAX_COLS = 5
    ...
    fuzzer_names = list(set(fuzzer for fuzzers_data_item in fuzzers_data for fuzzer in fuzzers_data_item.keys()))
    fuzzer_names = sorted(fuzzer_names)
    fuzzer_colors = {fuzzer_name: colormap(i / 10) for i, fuzzer_name in enumerate(fuzzer_names)}
    ...
    end_col = start_col + 2  # The plot spans 2 columns
    ...
    plt.savefig(f"{output_dir_binary}/survival_plots.pdf", bbox_inches='tight')
    plt.close()
